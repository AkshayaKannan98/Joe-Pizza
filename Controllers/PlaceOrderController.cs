...
namespace Pizzaorder.Controllers
{
    public class PlaceOrderController : Controller
    {
        ...
        public PlaceOrderController(PizzaDbContext context)
        {
            _context = context;
        }
        public ActionResult PlaceOrder(int id, string val)
        {
            ...
            if (val == "Add")
            {
                ...
                var pizza = _context.pizzas.Where(s => s.Pno == id).ToList();
                if (pizza.Count > 0)
                    model = pizza[0];
            ...
            else if (val == "Remove")
            {
                ...
                {
                    var pizzalst = lstOlddata.Where(item => item.Pno == id).ToList();
                    if (pizzalst.Count > 0)
                    {
                        PizzaModel lapmodel = pizzalst[0];
